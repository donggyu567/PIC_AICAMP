...
def test_candidate_coverage_includes_300m_sorts_ids_and_keeps_empty_candidate(tmp_path):
    ...
    result, report = build_candidate_coverage(grid_path, candidate_path)
    repeated, repeated_report = build_candidate_coverage(grid_path, candidate_path)
...
def test_duplicate_candidate_id_is_invalid(tmp_path):
    ...
    with pytest.raises(ValueError, match="candidate_id"):
        build_candidate_coverage(grid_path, candidate_path)
